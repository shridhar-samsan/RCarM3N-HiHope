/*
 Image file name is MetalNeedle.tga
 Image width is 217 pixel.
 Image heigth is 26 pixel.
 Image colour depth is 32 bit (or 4 bytes per pixel).
 Image doesn't contain a colour palette (index).
 RLE image data start at byte 18 and end at byte 22637.
*/
...
static uint8_t *locClusterNeedleDataPtr = (uint8_t*)0;

const Img_t ClusterNeedle = {
    "cluster_needle.bin",
    217,
    26,
    IMG_ARGB8888,
