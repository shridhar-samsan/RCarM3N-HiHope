...
static uint32_t reg_ddrphy_read(uint32_t phyno, uint32_t regadd)
{
	...
	if ((Prr_Product != PRR_PRODUCT_M3N) && (Prr_Product != PRR_PRODUCT_V3H)) {
		...
		dsb_sev();
		while (mmio_read_32(DBSC_DBPDRGA(phyno)) != (regadd | 0x0000C000))
			dsb_sev();
...
static void reg_ddrphy_write(uint32_t phyno, uint32_t regadd, uint32_t regdata)
{
	...
	if ((Prr_Product != PRR_PRODUCT_M3N) && (Prr_Product != PRR_PRODUCT_V3H)) {
		...
		while (mmio_read_32(DBSC_DBPDRGA(phyno)) != (regadd | 0x00008000))
			dsb_sev();
		mmio_write_32(DBSC_DBPDRGA(phyno), regadd | 0x00008000);
...
static uint32_t set_term_code(void)
{
	...
	term_code = ddrtbl_getval(_cnf_DDR_PHY_ADR_G_REGSET, _reg_PHY_PAD_DATA_TERM);
	override = 0;
	for (i = 0; i < 2; i++)
...
void ddr_padcal_tcompensate_getinit(uint32_t override)
{
	...
	if (!override) {
		...
		if (dataL < 2800)
			tcal.init_temp = (143 * (int32_t)dataL - 359000) / 1000;
		else
			tcal.init_temp = (121 * (int32_t)dataL - 296300) / 1000;
