...
int fdtdec_setup_memory_banksize_fdt(const void *blob)
{
	...
	for (bank = 0; bank < CONFIG_NR_DRAM_BANKS; bank++) {
		...
		/*
		 * Save DRAM bank size and their number in the gd variable as the bd
		 * is not initialized on the early board booting stage.
		 */
		gd->dram_bank_size = (phys_size_t)(res.end - res.start + 1);
		if (!gd->dram_num_detected)
			gd->dram_bank_num++;
#endif
