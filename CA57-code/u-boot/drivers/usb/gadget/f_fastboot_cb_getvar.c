...
static int cb_getvar_snapshot_update_status(char *buf, int buf_size)
{
	enum MergeStatus status;
	/* Default 'unknown' */
	const char *status_str = "unknown";

	if (!virtual_ab_get_status(&status))
		return snprintf(buf, buf_size, status_str);

	switch (status) {
		case VIRTUAL_AB_NONE:
			status_str = "none";
			break;
		case VIRTUAL_AB_SNAPSHOTED:
			status_str = "snapshotted";
			break;
		case VIRTUAL_AB_MERGING:
			status_str = "merging";
			break;
		case VIRTUAL_AB_CANCELLED:
			status_str = "canceled";
			break;

		case VIRTUAL_AB_UNKNOWN:
			/* Fallthrough */
		default:
			/* Default 'unknown' */
			break;
...
#if CONFIG_IS_ENABLED(ANDROID_VIRTUAL_AB_UPDATE)
	{ "snapshot-update-status", cb_getvar_snapshot_update_status },
#endif
