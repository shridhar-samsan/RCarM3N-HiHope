...
#include <tee.h>
typedef enum {
	RECEIVE_BINDING_DATA = 0,
	SEND_BINDING_DATA = 1,
} keystore_ext_command_t;
#define KEYSTORE_EXTENSION_UUID \
		{ 0x96757ec7, 0x50bc, 0x4502, \
			{ 0x80, 0xa3, 0x99, 0xd0, 0x2e, 0x40, 0x27, 0x6a } }
#endif
...
int avb_main(int boot_device, char *argv[MAX_BOOTI_ARGC])
{
	...
	case AVB_AB_FLOW_RESULT_OK:
		...
		if (vbmeta_buf == NULL) {
			avb_fatal("Failed allocate vbmeta buffer memory\n");
			break;
		}
		...
		if (!avb_str_concat(full_partition_name,
				...
				slot_data->ab_suffix,
				avb_strlen(slot_data->ab_suffix))) {
			avb_fatal("Failed construct vbmeta full partition name\n");
			break;
		}
		...
		ret = read_from_part(CONFIG_FASTBOOT_FLASH_MMC_DEV,
				...
				vbmeta_buf, &vbmeta_num_read);
		if (ret) {
			avb_fatal("Failed read vbmeta partition\n");
			break;
		}

		/* Before using this function, MUST verify |image_data| with
		...
		uint32_t MMmmss = 0;
		uint32_t YYYYMMDD[3] = { 0, 0, 0 };

		/*  Descriptors from "vbmeta.img":
		 *     Prop: com.android.build.boot.os_version -> '11'
		 *     Prop: com.android.build.boot.security_patch -> '2021-02-05'
		 *     Prop: com.android.build.system.os_version -> '11'
		 *     Prop: com.android.build.system.security_patch -> '2021-02-05'
		 *     Prop: com.android.build.vendor.security_patch -> '2021-02-05'
		 *  These properties MUST be aligned with /build/make/core/Makefile
		 */
		 ...
		 * two-digit day of the last update. For example, for a key generated on an Android
		 * device last updated on June 5, 2018, the value would be 20180605.
		 * If the day is not known, 00 may be substituted.
		 */
		out_value = avb_property_lookup(vbmeta_buf, vbmeta_size,
				"com.android.build.boot.security_patch", 0, &out_value_size);
		if (out_value_size) {
			/* Format: '2021-02-05' */
			YYYYMMDD[0] = (uint32_t)simple_strtol(out_value, NULL, 10) * 10000;
			if ((out_value = strchr(out_value, '-')) != NULL) {
				YYYYMMDD[0] += (uint32_t)simple_strtol(out_value + 1, NULL, 10) * 100;
				if ((out_value = strchr(out_value + 1, '-')) != NULL) {
					YYYYMMDD[0] += (uint32_t)simple_strtol(out_value + 1, NULL, 10);
				}
			}
		}

		out_value = avb_property_lookup(vbmeta_buf, vbmeta_size,
				"com.android.build.vendor.security_patch", 0, &out_value_size);
		if (out_value_size) {
			YYYYMMDD[2] = (uint32_t)simple_strtol(out_value, NULL, 10) * 10000;
			if ((out_value = strchr(out_value, '-')) != NULL) {
				YYYYMMDD[2] += (uint32_t)simple_strtol(out_value + 1, NULL, 10) * 100;
				if ((out_value = strchr(out_value + 1, '-')) != NULL) {
					YYYYMMDD[2] += (uint32_t)simple_strtol(out_value + 1, NULL, 10);
				}
			}
		}

		/* The value of the tag is an integer of the form YYYYMM,
		 * where YYYY is the four-digit year of the last update
		 * and MM is the two-digit month of the last update.
		 */
		out_value = avb_property_lookup(vbmeta_buf, vbmeta_size,
				"com.android.build.system.security_patch", 0, &out_value_size);
		if (out_value_size) {
			YYYYMMDD[1] = (uint32_t)simple_strtol(out_value, NULL, 10) * 100;
			if ((out_value = strchr(out_value, '-')) != NULL) {
				YYYYMMDD[1] += (uint32_t)simple_strtol(out_value + 1, NULL, 10);
				/* Note: ignore DD */
			}
		}

		/* The value of the tag is an integer of the form MMmmss,
		 * where MM is the major version number, mm is the minor version number,
		 * and ss is the sub-minor version number. For example, for a key generated
		 * on Android version 4.0.3, the value would be 040003.
		 */
		out_value = avb_property_lookup(vbmeta_buf, vbmeta_size,
				"com.android.build.system.os_version", 0, &out_value_size);
		if (out_value_size) {
			/* Format: '11.0.0' */
			MMmmss = (uint32_t)simple_strtol(out_value, NULL, 10) * 10000;
			if ((out_value = strchr(out_value, '.')) != NULL) {
				MMmmss += (uint32_t)simple_strtol(out_value + 1, NULL, 10) * 100;
				if ((out_value = strchr(out_value + 1, '.')) != NULL) {
					MMmmss += (uint32_t)simple_strtol(out_value + 1, NULL, 10);
				}
			}
		}

		/* Pass binding data to the TA */
#ifdef CONFIG_OPTEE
		struct udevice *tee = NULL;
		struct tee_open_session_arg arg_ses;
		const struct tee_optee_ta_uuid uuid = KEYSTORE_EXTENSION_UUID;
		struct tee_invoke_arg arg;
		struct tee_param param[4];

		tee = tee_find_device(tee, NULL, NULL, NULL);
		if (!tee) {
			avb_fatal("Failed to find tee device.");
			break;
		}

		memset(&arg_ses, 0, sizeof(arg_ses));
		tee_optee_ta_uuid_to_octets(arg_ses.uuid, &uuid);
		ret = tee_open_session(tee, &arg_ses, 0, NULL);
		if (ret != 0 || arg_ses.ret != 0) {
			avb_fatal("Failed to open session with keystore_ext.");
			break;
		}
		...
		param[0].u.value.a = MMmmss;
		param[1].u.value.a = YYYYMMDD[0];
		param[2].u.value.a = YYYYMMDD[1];
		param[3].u.value.a = YYYYMMDD[2];

		arg.func = RECEIVE_BINDING_DATA;
		arg.session = arg_ses.session;

		ret = tee_invoke_func(tee, &arg, ARRAY_SIZE(param), param);
		if (ret != 0 || arg.ret != 0) {
			avb_fatal("Failed to invoke command to keystore_ext.");
