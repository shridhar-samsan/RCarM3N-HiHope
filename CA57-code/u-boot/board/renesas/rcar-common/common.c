...
/*
 * Init amount of memory at the first stage from
 * BL2 dtb, and use u-boot dtb after this.
 */
static int mem_init_from_bl2 = 1;
...
int dram_init_banksize(void)
{
	...
	/* Check if ATF passed us DTB. If not, fall back to builtin DTB. */
	if (fdt_magic(atf_fdt_blob) == FDT_MAGIC && mem_init_from_bl2)
		blob = atf_fdt_blob;
	else
		blob = gd->fdt_blob;
