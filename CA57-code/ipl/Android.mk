...
ifneq (,$(filter $(TARGET_DEVICE), salvator ulcb kingfisher))
...
IPL_SA_SRC          := $(abspath ./device/renesas/bootloaders/ipl/tools/renesas/rcar_layout_create)
...
IPL_SA_SRC_ABS      := $(IPL_SA_OUT_ABS)/tools/renesas/rcar_layout_create

# SA0 SA6 SREC (HyperFlash) build output
IPL_SA_HF_OUT_ABS   := $(PRODUCT_OUT_ABS)/obj/IPL_SA_HF_OBJ
IPL_SA_HF_SRC_ABS   := $(IPL_SA_HF_OUT_ABS)/tools/renesas/rcar_layout_create

ifeq ($(DEBUG),1)
    IPL_BUILD := debug
endif

IPL_SA0_BINARY      := $(IPL_SA_OUT_ABS)/tools/renesas/rcar_layout_create/bootparam_sa0.bin
IPL_SA6_BINARY      := $(IPL_SA_OUT_ABS)/tools/renesas/rcar_layout_create/cert_header_sa6.bin
IPL_BL2_BINARY      := $(IPL_OUT_ABS)/rcar/$(IPL_BUILD)/bl2.bin
IPL_BL31_BINARY     := $(IPL_OUT_ABS)/rcar/$(IPL_BUILD)/bl31.bin

IPL_SA0_SREC        := $(IPL_SA_HF_OUT_ABS)/tools/renesas/rcar_layout_create/bootparam_sa0.srec
IPL_SA6_SREC        := $(IPL_SA_HF_OUT_ABS)/tools/renesas/rcar_layout_create/cert_header_sa6.srec
...
# HiHope boards don't have ROHM BD9571 PMIC
# Pass board type flag to platform.mk
ifeq ($(BOARD_HIHOPE),true)
PLATFORM_FLAGS += \
    BOARD_HIHOPE=1
endif
...
ipl_sa:
	...
	mkdir -p $(IPL_SA_OUT_ABS)/tools
	cp -R $(IPL_SRC)/tools/renesas $(IPL_SA_OUT_ABS)/tools
	cp -R $(IPL_SRC)/make_helpers $(IPL_SA_OUT_ABS)/
...
ipl_sa_hf:
	...
	mkdir -p $(IPL_SA_HF_OUT_ABS)/tools
	cp -R $(IPL_SRC)/tools/renesas $(IPL_SA_HF_OUT_ABS)/tools
	cp -R $(IPL_SRC)/make_helpers $(IPL_SA_HF_OUT_ABS)/
...
endif # TARGET_DEVICE salvator ulcb kingfisher
