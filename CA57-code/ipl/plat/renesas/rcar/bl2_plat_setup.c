...
# define NOTICE(...)	do { if (*((uint64_t *)RCAR_SRAM_STASH) != RCAR_SRAM_NICK) { printf("NOTICE:  " __VA_ARGS__); } } while(0)
#else
# define NOTICE(...)
#endif
 ...
 */
static const uint64_t BL2_RO_BASE		= (unsigned long)(&__RO_START__);
static const uint64_t BL2_RO_LIMIT		= (unsigned long)(&__RO_END__);

#if USE_COHERENT_MEM
static const uint64_t BL2_COHERENT_RAM_BASE	= (unsigned long)(&__COHERENT_RAM_START__);
static const uint64_t BL2_COHERENT_RAM_LIMIT	= (unsigned long)(&__COHERENT_RAM_END__);
#endif
...
#if (RCAR_BOOT_EMMC == 1)
	reg &= ~MODEMR_BOOT_DEV_MASK;
	reg |= MODEMR_BOOT_DEV_EMMC_50X8;
#endif
...
static void bl2_advertise_dram_size(uint32_t product)
{
	...
	const uint32_t product_prr_cut __attribute__((unused)) = mmio_read_32(PRR) & PRR_CUT_MASK;
...
#elif (RCAR_DRAM_LPDDR4_MEMCONF == 3)
		if (cnf_boardtype == 7) {
			/* 4GB(1GBx4) */
			dram_config[1] = 0x40000000ULL;
			dram_config[3] = 0x40000000ULL;
	...
	case PRR_PRODUCT_M3:
		if (product_prr_cut == PRR_PRODUCT_30) {
			/* 8GB(4GBx2 4ch split) for M3 Ver. 3.0*/
			dram_config[1] = 0x100000000ULL;
			dram_config[5] = 0x100000000ULL;
		} else {
			/* 4GB(2GBx2) for M3 Ver. 1.X */
			dram_config[1] = 0x80000000ULL;
			dram_config[5] = 0x80000000ULL;
		}
		break;
	...
	case PRR_PRODUCT_M3N:
		...
		/* 4GB(4GBx1) - HiHope M3N configuration */
		dram_config[1] = 0x100000000ULL;
#elif (RCAR_DRAM_LPDDR4_MEMCONF == 1)
		/* 2GB(1GBx2) */
		dram_config[1] = 0x80000000ULL;
